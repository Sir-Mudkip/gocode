...
tests=()
tests+=("TestAdd" "Add" "2" "3")
tests+=("TestSubtract" "Subtract" "5" "3")
tests+=("TestMultiply" "Multiply" "2" "3")
tests+=("TestDivideValid" "Divide" "6" "3")
tests+=("TestDivideByZero" "Divide" "6" "0")
...
do
  ...
  if [ "$result" -eq 0 ]; then
      echo "<failed> There is no function called '$name'"
      exit 1
  fi
