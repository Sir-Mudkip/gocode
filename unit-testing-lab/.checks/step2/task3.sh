...
query='[.[] | select(.Package == "getting-started-with-go-unit-testing/step2" and .Test == "TestCalculateArea" and .Action == "output" and (.Output | startswith("CalculateArea called with parameters 3 and 5")))] | length'
result=$(jq "$query" <<< "$json")
