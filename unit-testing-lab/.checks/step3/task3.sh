...
# Were error messages printed?
query='[.[] | select(.Package == "getting-started-with-go-unit-testing/step3" and .Test == "TestCalculateAreaNegativeLength" and .Action == "output" and (.Output | contains("area_test.go")))] | length'
result=$(jq "$query" <<< "$json")

if [ "$result" -eq 0 ]; then
    echo "<failed> No error messages were printed. Did you use t.Error() or t.Errorf()?"
    exit 1
fi
