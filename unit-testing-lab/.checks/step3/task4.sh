...
if [ "$result" -eq 0 ]; then
    echo "<failed> The test failed, but it should have passed; make sure that the 'CalculateArea' function has the following line: 'if length <= 0 { return 0, errors.New(\"length is too low\") }'"
    exit 1
fi
